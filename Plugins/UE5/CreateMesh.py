...
def spawnActor(path,location,scale,rotation):
    ...
    actor.set_actor_rotation(actor_rotation,True)
    return actor
...
def executeSlowTask():
    ...
    with unreal.ScopedSlowTask(quantity_steps_in_slow_task,"My Slow Task Text ...") as slow_task:
        ...
        for x in range(quantity_steps_in_slow_task):
            ...
            actor = spawnActor(data['meshPath'][x],P[x],data['scale'][x],data['rotation'][x])
