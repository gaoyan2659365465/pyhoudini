...
class W_ScrollArea(QWidget):
    def __init__(self,parent=None):
        ...
        self.v_layout.setContentsMargins(2,0,2,0)
        self.v_layout.setSpacing(0)
        
        self.item_widget = QWidget(self)
        self.item_widget.setLayout(self.v_layout)
    ...
    def addItem(self,item):
        """添加子项"""
        self.v_layout.addWidget(item,Qt.AlignTop)
    ...
    def removeAllItem(self):
        ...
        for i in range(self.v_layout.count()):
            item = self.v_layout.itemAt(i).widget()
            items.append(item)
        for item in items:
