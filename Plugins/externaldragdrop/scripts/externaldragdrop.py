...
def copImages(network, ctx, filename):
	if not matchTypes(IMAGE_EXTENSIONS).search(filename.lower()):
		return
...
def chanFiles(network, ctx, filename):
	if not matchTypes(CHAN_EXTENSIONS).search(filename.lower()):
		return
...
def shopImages(network, ctx, filename):
	...
	elif ctx.type().name() == 'redshift_vopnet':
		image = ctx.createNode('TextureSampler', node_name=name)
		image.parm('tex0').set(filename)

	elif ctx.type().name() == 'arnold_vopnet':
		image = ctx.createNode('image', node_name=name)
		image.parm('filename').set(filename)

	elif ctx.type().name() == 'octane_vopnet':
		image = ctx.createNode('octane::NT_TEX_IMAGE', node_name=name)
		image.parm('A_FILENAME').set(filename)
...
def objGeom(network, ctx, filename):
	if not matchTypes(GEO_EXTENSIONS).search(filename.lower()):
		return
...
def hdaAsset(network, ctx, filename):
	if not matchTypes(ASSET_EXTENSIONS).search(filename.lower()):
		return
